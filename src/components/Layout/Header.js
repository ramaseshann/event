...
const Header = () => {
   ...
   function onClick(){
    signOut(auth).then(()=>{checkAuthentication(); console.log("Logged out")}).catch((error)=>{console.log(error)})
  ...
  return (
    ...
    <div className='container  flex items-center justify-between px-[100px] border-2'>
        ...
        {user && <div className='flex items-center justify-between cursor-pointer'>
            ...
            <div className='flex items-center justify-center cursor-pointer rounded-full h-10 text-red-800 ' onClick={onClick}>  
            {user === null ?"" : "Sign Out"}
            </div> 
            <div>
